...
class _CACHE {
    using _CACHE_BLOCK = std::array<_DATA_BLOCK<SZ_TAG_BITS, SZ_BLOCK_SIZE>, SZ_SET_ASSO>;
...
public:
    explicit _CACHE(unsigned int l = 0, _CACHE *p = nullptr, _CACHE *c = nullptr) : latency(l), parent(p), child(c) {
        if (SZ_BLOCK_SIZE % 8 != 0)
            throw std::logic_error("SZ_SET_ASSO is not in BYTES.");
        std::get<0>(this->field_partition) = SZ_TAG_BITS;
        std::get<2>(this->field_partition) = round(log2(SZ_SET_ASSO));
        ...
        if (SZ_TAG_BITS != (32 - std::get<2>(this->field_partition) - std::get<1>(this->field_partition)))
            throw std::logic_error("SZ_TAG_BITS doesn't match SZ_SET_ASSO and SZ_NUM_BLOCK");
    }
