...
private:
	...
	std::array<bool, 8> ready{false, false, false, false, false, false, false};//1 means ready
...
public:
	...
	uint32_t getMissRate() {
		uint64_t sum = (this->hit_miss_count.first + this->hit_miss_count.second);
		uint32_t miss_rate = 100 * (this->hit_miss_count.first / sum);
		return miss_rate;
	}

	uint32_t getHitRate() {
		uint64_t sum = (this->hit_miss_count.first + this->hit_miss_count.second);
		uint32_t hit_rate = 100 * (this->hit_miss_count.second / sum);
		return hit_rate;
	}
