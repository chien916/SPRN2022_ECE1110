...
//TO-DO HERE: Should there be a read from parent cache?
	...
	void reportSchedule(const Task &_task) {
		std::string task_type;
		if (_task.getTaskType() == task_t::task_readAddress)
			task_type = "READ";
		else if (_task.getTaskType() == task_t::task_writeAddress)
			task_type = "WRITE";
		else return;
		uint32_t address_raw = _task.getTaskValue();
		std::string address_bit = std::bitset<32>(address_raw).to_string();
		address_bit.erase(0, std::min(address_bit.find_first_not_of('0'), address_bit.size() - 1));
		std::string front_dashes;
		for (size_t i = 0; i < this->report_writer.second; i++) front_dashes += "\t";
		report_writer.first
				<< front_dashes
 ...
 */
	bool taskReadAddress(std::tuple<uint32_t, uint32_t, uint32_t> *_arguments) {
		...
		Task this_task(task_t::task_writeAddress, _address, _arrive_time);
		this->task_queue.insert(this_task);
		reportSchedule(this_task);
		return true;
	}
 ...
 */
	bool taskPrintCacheRate(std::tuple<uint32_t, uint32_t, uint32_t> *_arguments) {
		...
		uint32_t _arrive_time = std::get<1>(*_arguments);
		if (_cache_level > this->cache_count) return false;
		this->task_queue.insert(Task(task_t::task_reportHitMiss, _cache_level, _arrive_time));
		return true;
	}
 ...
 */
	bool taskPrintCacheImage(std::tuple<uint32_t, uint32_t, uint32_t> *_arguments) {
		...
		uint32_t _arrive_time = std::get<1>(*_arguments);
		if (_cache_level > this->cache_count) return false;
		this->task_queue.insert(Task(task_t::task_reportImage, _cache_level, _arrive_time));
		return true;
	}
 ...
 *//*
	...
	void runTaskQueue() {
		while (!task_queue.empty()) {
			Task current_task = *(task_queue.begin());
			if (current_task.getArriveTime() <= clock_count) {
				task_t this_task = current_task.getTaskType();
				uint32_t this_value = current_task.getTaskValue();
				uint32_t this_arrive_time = current_task.getArriveTime();
